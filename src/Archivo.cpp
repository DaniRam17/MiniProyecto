...
// También guarda accesos en un archivo de texto
// -------------------------------------------------------------
...
// ✅ Guarda los accesos del usuario en un archivo de texto
void Archivo::guardarAccesos(const Usuario &usuario, const string &nombreArchivo)
{
    ofstream archivo(nombreArchivo, ios::app);
    if (!archivo)
    {
        cerr << "No se pudo abrir archivo para guardar accesos." << endl;
        return;
    }
    ...
    while (actual)
    {
        archivo << "ID: " << usuario.getId()
                << ", Nombre: " << usuario.getNombre()
                << ", Acceso: " << actual->fechaHora << endl;
